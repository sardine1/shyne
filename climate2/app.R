...
ui <- fluidPage(theme = shinytheme("readable"),
      ...
      dashboardPage(skin = "black",
        ...
        dashboardSidebar(
          ...
          #menuItem("По месяцам", tabName = "temperature_yearly")),
          ...
          tabItems(
                    ...
                    fluidRow(
                      tabBox(
                        ...
                        tabPanel("Планы", h5("Планы по развитию"), br(), 
                                 ...
                                 icon("calendar"),
                                 "Ближайшее обновление запланировано на 22.06", br(),
                      ...
                      tabBox(
                        ...
                        tabPanel("Файл", h5("Информация по выгрузке файла"), br(),
                                 ...
                                 "19. pr24 - общее количество осадков за 24 часа;", br(), br(),
                                  ...
                                  "15. SnowDepcm – глубина снежного покрова в сантиметрах;", br(), br(),
            ...
            tabItem(tabName = "temperature_hourly",
                    ...
                    selectInput("station", label = "Станция:", 
                                ...
                                selected = "Novgorod"),
                    dateInput("date1", "Начальная дата:", min = "2001-01-01", value = "2022-01-01", startview = 'year'),
                    dateInput("date2", "Конечная дата:", min = "2001-01-01", value = "2022-03-31", startview = 'year'),
                    
                    actionButton("submitbutton", "Показать"),
                    br(),
                    
                    HTML("<h3>Скачать файл с данными</h3>"),
                    textInput("caption", "Наименование файла", value = "Climate_data_hourly"),
            ...
            tabItem(tabName = "temperature_daily",
                    box(width = "100%",
                        ...
                        selectInput("station0", label = "Станция:", 
                                    #станции
                                    choices = list("Belogorka" = "26069", "Nikolaevskoe" = "26167", "Volosovo" = "26067", "Lomonosov" = "26060",
                                                   "St.petersburg (Voejkovo)" = "26063", "Dno" = "26268",
                                                   "Ljuban" = "26078", "Novgorod" = "26179", "Petrokrepost" = "26072", "Kingisepp" = "26059",
                                                   ...
                                                   "Belozersk" = "22939", "Njandoma" = "22854", "Kargopol" = "22845", "Vladimir" = "27532"), 
                                    selected = "Novgorod"),
                        dateInput("date10", "Начальная дата:", value = "2022-02-21"),
                        dateInput("date20", "Конечная дата:", value = "2022-03-21"),
                        
                        HTML("<h3>Изменить параметр графика</h3>"),
                        
                        selectInput("param0", label = "Абсцисса",
                        ...
                        HTML("<h3>Скачать файл с данными</h3>"),
                        textInput("caption0", "Наименование файла", value = "Climate_data_daily"),
            ...
            tabItem(tabName = "temperature_monthly",
                    box(width = "100%",
                      HTML("<h3>Изменяемые параметры</h3>"),
                      selectInput("station1", label = "Станция:", 
                                  #станции
                                  choices = list("Belogorka" = "26069", "Nikolaevskoe" = "26167", "Volosovo" = "26067", "Lomonosov" = "26060",
                                                 "St.petersburg (Voejkovo)" = "26063", "Dno" = "26268",
                                                 "Ljuban" = "26078", "Novgorod" = "26179", "Petrokrepost" = "26072", "Kingisepp" = "26059",
                      ...
                      dateInput("date11", "Начальная дата:", value = "2021-02-01"),
                      h6("Начальная дата – первое число месяца"),
                      dateInput("date21", "Конечная дата:", value = "2021-12-31"),
                      ...
                      HTML("<h3>Скачать файл с данными</h3>"),
                      textInput("caption0", "Наименование файла", value = "Climate_data_montly"),
                      downloadButton("download_montly", "Скачать"),
                      
                      br(),
                      mainPanel(
                        tags$label(h3('График средней температуры по заданному интервалу времени')),
                        plotlyOutput("printplot_montly"),
                        tags$hr(),
                        tableOutput("printtable_montly")
                      ))
            ),
            
            #Четвертое вложение
            tabItem(tabName = "temperature_yearly",
                    box(
                      HTML("<h3>Изменяемые параметры</h3>"),
                      selectInput("station1", label = "Станция:", 
                                  #станции
...
server <- function(input, output) {
  ...
  output$printplot_daily <- renderPlotly({
    ...
    daf <- data.frame(
      Name = c("station0",
               ...
               "param0",
               "caption0"),
      Value = as.character(c(input$station0,
                             input$date10,
                             input$date20,
                             input$param0,
                             input$caption0)),
      ...
      df = meteo_ogimet(interval = "daily", date = c(input$date10, input$date20), station = input$station0)
      data1 = data.frame(df[names(df) == input$param0], df[2])
      if (input$param0 == "TemperatureCAvg") {
        fig1 = plot_ly(data1, x = ~ Date, y = ~ TemperatureCAvg) %>% 
          add_lines()
        
      } else if (input$param0 == "TemperatureCMax") {
        fig1 = plot_ly(data1, x = ~ Date, y = ~ TemperatureCMax) %>% 
          add_lines()
        fig1
      } else if (input$param0 == "TemperatureCMin") {
  ...
  # Plot2
  output$printplot_montly <- renderPlotly({
    input$submitbutton
    ...
    daf <- data.frame(
      Name = c("station1",
               "date11",
               "date21"),
      Value = as.character(c(input$station1,
                             input$date11,
                             input$date21)),
      stringsAsFactors = FALSE)
    
    df = meteo_ogimet(interval = "daily", date = c(input$date11, input$date21), station = input$station1)
  ...
  output$printtable_daily <- renderTable({  
    ...
    df0 = meteo_ogimet(interval = "daily", date = c(input$date10, input$date20), station = input$station0)
    
    df10 = df0[2]
    
    data <- data.frame(format(df10, format = "%Y %m %d"), df0[3:9], df0[11], df0[13:17])
    
    data.frame(format(df10, format = "%Y %m %d"), df0[3:9], df0[11], df0[13:17])
  ...
  output$download_daily <- downloadHandler(
    ...
    content = function(file) {write.csv(meteo_ogimet(interval = "daily", date = c(input$date10, input$date20), station = input$station0), file)}
    
  )
  
  # Data Table3
  output$printtable_montly <- renderTable({
    input$submitbutton
    
    df1 = meteo_ogimet(interval = "daily", date = c(input$date11, input$date21), station = input$station1)
  ...
  output$download_montly <- downloadHandler(
    ...
    content = function(file) {write.csv(
      hobofullmean <- meteo_ogimet(interval = "daily", date = c(input$date11, input$date21), station = input$station1) %>%
        tidyr::separate('Date',
                        into = c('year', 'month', 'day'),
                        sep= '-',
                        remove = FALSE) %>%
