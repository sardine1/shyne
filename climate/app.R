...
page <- fluidPage(theme = shinytheme("readable"),
                                ...
                                tabItems(
                                  ...
                                  tabItem(tabName = "temperature_yearly",
                                          box(width = "100%",
                                              ...
                                              selectInput("date_yearly_start", label = "Начальный год:", 
                                                          choices = list("2013" = "2013-01-01", "2014" = "2014-01-01", "2015" = "2015-01-01", "2016" = "2016-01-01",
                                              ...
                                              selectInput("date_yearly_end", label = "Конечный год:", 
                                                          choices = list("2013" = "2013-12-31", "2014" = "2014-12-31", "2015" = "2015-12-31", "2016" = "2016-12-31",
...
server <- function(input, output, session) {
  ...
  output$printplot_monthly <- renderPlotly({
    ...
    daf_daily <- data.frame(
      Name = c("date_monthly_start",
               "date_monthly_end",
               "caption_monthly"),
      Value = as.character(c(input$date_monthly_start,
  ...
  output$download_montly <- downloadHandler(
    ...
    content = function(file) {write.csv(
      hobofullmean <- meteo_ogimet(interval = "daily", date = c(input$date_monthly_start, input$date_monthly_end), station = as.numeric(subset(y, station == input$station_seek_monthly)[3])) %>%
        ...
        summarise(meantemp = mean(TemperatureCAvg, na.rm=TRUE), meantempCMax = mean(TemperatureCMax, na.rm=TRUE),
  ...
  output$printplot_yearly <- renderPlotly({
    ...
    daf_yearly <- data.frame(
      Name = c("date_yearly_start",
               "date_yearly_end",
               "caption_yearly"),
      Value = as.character(c(input$date_yearly_start,
